...
static uint32_t s_return_zero_move_start_tick;
static float    s_return_zero_from[JOINT_COUNT];
...
static const uint8_t s_define_zero[JOINT_COUNT] = { 1u, 1u, 0u };
...
static void define_absolute_zero_all(void)
{
    ...
    printf("  Put HIP and KNEE at mechanical 0 deg before powering the STM32.\r\n");
...
void legtest_run(void)
{
    ...
    for (;;)
    {
        ...
        if (!s_return_zero_active && (s_tick > LEGTEST_ARM_DELAY_MS))
        {
            ...
            else
            {
...
#if LEGTEST_GAIT_IDLE_AFTER
                        /* First make the gait-ending state genuinely IDLE.
                         * Nothing moves for the requested 2 second pause. */
                        s_stopped = 1;
                        disarm_all();
                        s_return_zero_active = 1u;
                        s_return_zero_armed = 0u;
                        s_return_zero_start_tick =
                            s_tick + LEGTEST_RETURN_ZERO_DELAY_MS;
                        printf("\r\n*** GAIT COMPLETE - all axes IDLE. "
                               "Returning to zero in %u s. ***\r\n",
                               (unsigned)(LEGTEST_RETURN_ZERO_DELAY_MS / 1000u));
#endif
#if LEGTEST_CAPTURE
                        cap_dump();
                ...
                for (int j = 0; j < JOINT_COUNT; j++)
                {
                     ...
                     *
                     * joint_cmd() carries the absolute answer: gait position
                     * about the mechanical zero defined by CAN 0x19, scaled
                     * to the drive's encoder shaft.
                     */
                    target[j]     = joint_cmd(j, all[s_gait_col[j]]);
                    target_vel[j] = all_vel[s_gait_col[j]] * vscale
                                    * s_cmd_scale[j] * s_vel_ff[j];
        ...
        /* -----------------------------------------------------------------
         * POST-GAIT RETURN TO ZERO
         * -----------------------------------------------------------------
         * The gait completion path above disarms immediately.  After the
         * requested dead time, re-arm the live axes and command a time-based
         * ramp from their measured post-gait positions to 0.0 turns.  Once
         * the ramp is complete, keep commanding zero so the leg is held there.
         */
        if (s_return_zero_active)
        {
            if (s_tick < s_return_zero_start_tick)
            {
                /* Deliberately send nothing while the axes are IDLE. */
            }
            else
            {
                if (!s_return_zero_armed)
                {
                    uint8_t can_rearm = 1u;

                    for (int j = 0; j < JOINT_COUNT; j++)
                    {
                        if (!s_joint_live[j]) { continue; }
                        s_return_zero_from[j] = s_joint[j].pos;
                    ...
                    if (!can_rearm)
                    {
                        printf("!! Cannot return to zero: an axis is not "
                               "healthy. Staying IDLE.\r\n");
                        s_return_zero_active = 0u;
                        s_arm_blocked = 1u;
                    }
                    else
                    {
                        ...
                        s_stopped = 0u;
                        s_return_zero_armed = 1u;
                        s_return_zero_move_start_tick = s_tick;
                        printf("*** 2 s pause complete - returning whole leg "
                               "to ZERO over %u s. ***\r\n",
                               (unsigned)(LEGTEST_RETURN_ZERO_MS / 1000u));
                    }
                }

                if (s_return_zero_armed)
                {
                    uint32_t elapsed = s_tick - s_return_zero_move_start_tick;
                    float a = (elapsed >= LEGTEST_RETURN_ZERO_MS)
                                  ? 1.0f
                                  : ((float)elapsed /
                                     (float)LEGTEST_RETURN_ZERO_MS);

                    for (int j = 0; j < JOINT_COUNT; j++)
                    {
                        if (!s_joint_live[j]) { continue; }

                        target[j] = s_return_zero_from[j] * (1.0f - a);
                        target_vel[j] = (elapsed >= LEGTEST_RETURN_ZERO_MS)
                                      ? 0.0f
                                      : (-s_return_zero_from[j] /
                                         ((float)LEGTEST_RETURN_ZERO_MS * 0.001f));
                    }

                    if (elapsed >= LEGTEST_RETURN_ZERO_MS)
                    {
                        if (elapsed == LEGTEST_RETURN_ZERO_MS)
                        {
                            /* Return-to-zero is complete: disarm all axes. */
                            printf("*** whole leg returned to ZERO - "
                                   "disarming all axes. ***\r\n");
                            disarm_all();
                            s_stopped = 1u;
                            s_return_zero_active = 0u;
