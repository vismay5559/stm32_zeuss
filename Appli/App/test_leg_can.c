...
/* Gait positions are now absolute joint angles about the manually defined zero. */
...
/*
 * Where the run has got to. Each phase is entered once, records the tick it
 * started on and the pose it started from, and runs on time from there - the
 * setpoint is never a function of the live encoder, because a setpoint
 * recomputed from the measurement can never build the error that a position
 * loop turns into torque.
 */
typedef enum
{
...
/*
 * The run is a sequence of timed phases, and these are their durations.
 *
 *   GOTO_ZERO    from wherever the leg powered up, to absolute zero
 *   ENTRY        from absolute zero, to the trajectory's first sample
 *   GAIT         the trajectory itself
 *   SETTLE       a silent pause, axes still armed and holding the last pose
 *   RETURN_ZERO  back to absolute zero, then disarm
 *
 * GOTO_ZERO is generous on purpose: it is the only move whose distance is not
 * known in advance. Everything after it starts from a defined pose.
 */
...
/*
 * Define the robot's mechanical zero over CAN.
 *
 * The user manually puts HIP and KNEE at their mechanical zero before booting.
 * ODrive CANSimple command 0x19 (Set_Absolute_Position) is then sent with a
 * float32 value of 0.0 turns. ODrive maps this command to pos_estimate and
 * enables absolute positioning; no firmware-specific SDO endpoint is needed.
 *
 * Only HIP and KNEE are written here. The ANKLE was already zeroed by the user
 * and must not be silently re-zeroed if it is physically elsewhere.
 *
 * We verify the result using the normal Get_Encoder_Estimates (0x009)
 * telemetry. If the drive does not report approximately zero after the
...
static uint8_t define_absolute_zero_one(int j)
{
    ...
    printf("  %-9s CAN 0x19: Set_Absolute_Position(0.0 turns)\r\n",
           s_joint_name[j]);

    if (!can_send(node, ODRV_CMD_SET_ABS_POS, data, 4u))
    {
        printf("  %-9s absolute zero: CAN TX FAILED\r\n",
               s_joint_name[j]);
        return 0u;
    }

    /*
     * 0x19 is a host->ODrive command, so there is no acknowledgement frame.
     * Wait for fresh encoder telemetry and verify the reported position.
     */
    uint8_t got_fresh = 0u;
    for (uint32_t wait = 0u; wait < 300u; wait += 10u)
    {
        HAL_Delay(10);
        legtest_on_rx();

        if (s_joint[j].n_encoder > old_n_encoder)
        {
            got_fresh = 1u;
            break;
        }
    ...
    if (!got_fresh)
    {
        printf("  %-9s absolute zero: no fresh encoder feedback\r\n",
               s_joint_name[j]);
        return 0u;
    }
...
static void define_absolute_zero_all(void)
{
    ...
    for (int j = 0; j < JOINT_COUNT; j++)
    {
        if (!s_define_zero[j]) { continue; }
        if (!define_absolute_zero_one(j)) { s_absolute_ref_ok = 0u; }
    }

    if (!s_absolute_ref_ok)
    {
        s_arm_blocked = 1u;
        printf("!! absolute reference setup FAILED - CLOSED LOOP BLOCKED\r\n");
    }
    else
    {
...
/*
 * Enter a phase, recording when it started and where each joint was.
 *
 * Capturing the starting pose here is what lets every ramp be a straight line
 * in time rather than a chase of the measurement, and it is captured ONCE so a
 * joint that lags does not drag the whole ramp out behind it.
 */
static void phase_enter(run_phase_t next, const char *what)
{
    for (int j = 0; j < JOINT_COUNT; j++)
    {
        s_phase_from[j] = s_joint[j].pos;
    }
...
/* 0.0 at the start of the current phase, 1.0 once `ms` has elapsed. */
static float phase_alpha(uint32_t ms)
{
    uint32_t elapsed = s_tick - s_phase_tick;

    if (elapsed >= ms) { return 1.0f; }
    return (float)elapsed / (float)ms;
}

/*
 * A straight line from where the phase started to `to`, in drive turns, with
 * the constant velocity that line implies.
 *
 * The velocity is the whole distance over the whole ramp, not a per-tick
 * difference: the drive is being told what the setpoint is about to do, which
 * is the only useful thing feedforward can say.
 */
static void ramp_to(int j, float to, uint32_t ms, float a,
                    float *target, float *target_vel)
{
    float from = s_phase_from[j];

    target[j] = from + (to - from) * a;
...
void legtest_run(void)
{
    ...
    for (;;)
    {
         ...
         * -----------------------------------------------------------------
         * WAIT_ARM -> GOTO_ZERO -> ENTRY -> GAIT -> SETTLE -> RETURN_ZERO
         *          -> DONE
         *
         * The trajectory is ABSOLUTE (LEGTEST_GAIT_RELATIVE 0): a table value
         * is the joint angle, measured from the zero the drives already hold.
         * Hip and knee get that zero from their own saved reference frame, the
         * ankle from the 0x19 sent at boot - either way the firmware starts by
         * driving to it, so every run begins from the same pose and a capture
         * from today is comparable with one from last week.
         *
         * Every phase is a straight line in TIME between two known poses. None
         * of them recompute the setpoint from the live encoder: a setpoint that
         * follows the measurement can never build an error, and error is what
        ...
        if (!s_stopped && !s_run_blocked)
        {
            ...
            case RUN_WAIT_ARM:
                /*
                 * Nothing is commanded during the countdown. The axes are
                 * armed by the block further down once they report healthy;
                 * the first move only starts when they have.
                 */
                if (s_tick > LEGTEST_ARM_DELAY_MS)
                {
                    ...
                    for (int j = 0; j < JOINT_COUNT; j++)
                    {
                        ...
                        if ((s_joint[j].axis_state !=
                             ODRV_AXIS_STATE_CLOSED_LOOP) ||
                            (s_joint[j].n_encoder == 0u))
                        {
                            ready = 0u;
                        }
                    }
                    ...
                    if (ready)
                    {
                        ...
                        {
                            s_run_blocked = 1u;
                            printf("\r\n!! trajectory does not fit the joint "
                                   "limits - NOT MOVING\r\n\r\n");
                        }
                        else
                        {
                            phase_enter(RUN_GOTO_ZERO,
                                        "GOING TO ABSOLUTE ZERO");

                            for (int j = 0; j < JOINT_COUNT; j++)
                            {
                                if (!s_joint_live[j]) { continue; }
                                printf("    %-9s %+7.2f deg -> 0.00\r\n",
                                       s_joint_name[j],
                                       (double)(s_phase_from[j] /
                                                s_cmd_scale[j] * 360.0f));
                            }
                        }
                    }
                }
            ...
            {
                /*
                 * The only move whose distance is not known in advance, since
                 * it starts from however the leg was left. Straight to zero,
                 * slowly, and everything after this begins from a defined pose.
                 */
                float a = phase_alpha(LEGTEST_GOTO_ZERO_MS);

                for (int j = 0; j < JOINT_COUNT; j++)
                {
                    ramp_to(j, 0.0f, LEGTEST_GOTO_ZERO_MS, a,
                            target, target_vel);
                }

                if (a >= 1.0f)
                {
                    phase_enter(RUN_ENTRY, "AT ZERO - ramping to the "
                                           "trajectory start");
                }
                break;
            }
            ...
            case RUN_ENTRY:
            {
                /* Absolute zero to the trajectory's first sample. */
                float a = phase_alpha(LEGTEST_GAIT_ENTRY_MS);

                for (int j = 0; j < JOINT_COUNT; j++)
                {
                    ramp_to(j, first[j], LEGTEST_GAIT_ENTRY_MS, a,
                            target, target_vel);
                }
            ...
            case RUN_GAIT:
            {
                float t = (float)(s_tick - s_phase_tick) * 0.001f;
                ...
                gait_sample_vel(fmodf(s_gait_phase, 1.0f), all, all_vel);

                for (int j = 0; j < JOINT_COUNT; j++)
                {
                    /*
                     * The trajectory's POSITION, absolute, about the zero the
                     * drives hold - not an integral of its velocity seeded
                     * from the encoder. A command built from the measurement
                     * has no reference in it anywhere, so noise and overshoot
                     * are folded back in and kept; the hip once integrated
                     * itself to 64 turns that way against a 1.8-turn gait.
                     */
                    target[j] = joint_cmd(j, all[s_gait_col[j]]);

                    target_vel[j] = s_gait_done
                                        ? 0.0f
                                        : (all_vel[s_gait_col[j]] *
                                           LEGTEST_GAIT_SPEED *
                ...
                if (s_gait_done)
                {
                    ...
                    cap_dump();
#endif
                    phase_enter(RUN_SETTLE, "GAIT COMPLETE - holding");
                }
                break;
            }
            ...
            case RUN_SETTLE:
                /*
                 * Hold the last commanded pose, still armed. A moment of
                 * nothing-changing before the return, so the leg is at rest
                 * rather than being handed a new ramp mid-motion.
                 */
                for (int j = 0; j < JOINT_COUNT; j++)
                {
                    target[j]     = s_phase_from[j];
                    target_vel[j] = 0.0f;
                }

                if (phase_alpha(LEGTEST_SETTLE_MS) >= 1.0f)
                {
            ...
            {
                float a = phase_alpha(LEGTEST_RETURN_ZERO_MS);

                for (int j = 0; j < JOINT_COUNT; j++)
                {
                    ramp_to(j, 0.0f, LEGTEST_RETURN_ZERO_MS, a,
                            target, target_vel);
                }
        ...
        /* Absolute-position hard stop: never transmit a target outside the
           configured joint range. Preflight limits_ok() catches the planned
           gait, while this catches any unexpected target generation/runtime
           fault before it reaches the drive. */
        if (!s_stopped && s_absolute_ref_ok)
        {
            for (int j = 0; j < JOINT_COUNT; j++)
            {
                if (!s_joint_live[j]) { continue; }

                float out_deg = (target[j] / s_cmd_scale[j]) * 360.0f;
                float abs_deg = (out_deg >= 0.0f) ? out_deg : -out_deg;

                if (abs_deg > s_limit_deg[j])
                {
                    printf("\r\n!! HARD LIMIT: %s target %+.2f deg exceeds +/-%.1f deg - DISARM\r\n",
                           s_joint_name[j], (double)out_deg,
                           (double)s_limit_deg[j]);
                    s_stopped = 1u;
                    disarm_all();
                    break;
                }
